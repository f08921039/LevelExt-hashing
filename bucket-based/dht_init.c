...
static int init_eh_table() {
    ...
    for (i = 0; i < (1 << INITIAL_EH_L_DEPTH); ++i) {
        ...
        ent1 = make_eh_dir_ent1(seg_array++, INITIAL_EH_L_DEPTH);
...
static void init_background_contex() {
    ...
    bg_context.chunk_reclaim.rclist_head = bg_context.chunk_reclaim.rclist_tail
                            = bg_context.chunk_reclaim.wait_rclist = NULL;
    bg_context.page_reclaim.rclist_head = bg_context.page_reclaim.rclist_tail
                            = bg_context.page_reclaim.wait_rclist = NULL;
}
