...
further_next_migrate_eh_slot :
	...
	if (eh_seg_low(header))
		...
		seg2 = add_eh_new_segment(&split, seg2, bucket, kv, 1, 0);
...
__attribute__((always_inline))
static int migrate_eh_undo_slot_no_further(
				struct eh_bucket *bucket,
				struct eh_bucket *high_bucket, 
				int bucket_id, int depth, 
				u16 *dest_ind, u16 *undo_ind, 
				int undo_count) {
	EH_BUCKET_SLOT slot_val, *slot_addr;
	int i, d_id;
	...
	for (i = 0; i < undo_count; ++i) {
		...
		if (dest_ind[d_id] >= EH_SLOT_NUM_PER_BUCKET || 
				unlikely(migrate_eh_int_slot(&high_bucket[d_id], slot_addr, 
									slot_val, depth, &dest_ind[d_id])) != 0)
			return -1;
	}
...
static int split_eh_bucket(
			struct eh_segment *target_seg,
			struct eh_two_segment *middle_seg, 
			struct eh_four_segment *dest_seg,
			EH_BUCKET_HEADER middle_header, 
			int bucket_id, int depth) {
	EH_BUCKET_HEADER b_header;
	struct eh_bucket *bucket, *bucket_m, *bucket_h;
	EH_BUCKET_SLOT slot_val, *slot_addr;
	int undo_count, s_id, d_id;
	u16 dest_ind[4];
	u16 undo_ind[4];

	bucket = &target_seg->bucket[bucket_id];

	b_header = READ_ONCE(bucket->header);

	if (unlikely(eh_seg_migrating_or_splited(b_header) || !cas_bool(
			&bucket->header, b_header, set_eh_seg_migrating(b_header))))
		return -1;
	
	bucket_m = &middle_seg->bucket[MUL_2(bucket_id, 1)];
	prefetch_eh_split_middle_bucket(middle_seg, bucket_id);

	bucket_h = &dest_seg->bucket[MUL_2(bucket_id, 2)];
	...
	for (s_id = 0; s_id < EH_SLOT_NUM_PER_BUCKET; ++s_id) {
		...
		if (eh_slot_ext(slot_val)) {
			...
			if (unlikely(dest_ind[d_id] >= EH_SLOT_NUM_PER_BUCKET || 
				migrate_eh_ext_slot(&bucket_h[d_id], slot_addr, 
									slot_val, &dest_ind[d_id]) == -1))
				goto split_eh_bucket_complete;
		...
		if (unlikely(migrate_eh_undo_slot_no_further(bucket, bucket_h, 
					bucket_id, depth, &dest_ind[0], &undo_ind[0], 4) != 0))
			goto split_eh_bucket_complete;

		undo_count = 0;
	}

	if (undo_count && unlikely(
			migrate_eh_undo_slot_no_further(bucket, bucket_h, bucket_id, 
						depth, &dest_ind[0], &undo_ind[0], undo_count) != 0))
		goto split_eh_bucket_complete;

	b_header = set_eh_seg_splited(b_header);

split_eh_bucket_complete :
	cas(&bucket_m[0].header, INITIAL_EH_BUCKET_HEADER, middle_header);
	cas(&bucket_m[1].header, INITIAL_EH_BUCKET_HEADER, middle_header);
...
static struct eh_two_segment *split_eh_segment(
						...
						u64 hashed_prefix,
						int depth) {
	EH_BUCKET_HEADER header, b_header, m_header, b_header2, h;
	...
	header = clear_eh_seg_migrating_and_splited(header);
	b_header = set_eh_seg_splited(header);
	b_header2 = set_eh_seg_migrating(header);
	...
	for (b_id = 0; b_id < EH_BUCKET_NUM; ++b_id) {
		...
		if (unlikely(eh_seg_migrating(h) || 
				!cas_bool(&bucket->header, h, b_header2)))
			return NULL;
		...
		cas(&bucket_m[0].header, INITIAL_EH_BUCKET_HEADER, m_header);
		cas(&bucket_m[1].header, INITIAL_EH_BUCKET_HEADER, m_header);
...
struct eh_two_segment *add_eh_new_segment(
				...
				int do_migrate) {
	EH_BUCKET_HEADER old, new, header;
	...
	if (unlikely((void *)s_ent == MAP_FAILED))
		ret_seg2 = (struct eh_two_segment *)MAP_FAILED;
	else {
	...
	if (unlikely((void *)ret_seg2 == MAP_FAILED))
		return (struct eh_two_segment *)MAP_FAILED;

eh_new_segment_existed :
	if (do_migrate)
		prefetch_eh_split_high_bucket(already_seg, bucket_id);

	if (unlikely(try_init_eh_seg_new_slot(
			already_seg, kv, split->hashed_key, split->depth)))
		do_migrate = 0;
	else {
		ret_seg2 = NULL;
...
finish_add_eh_new_segment :
	...
	if (do_migrate && likely(split_eh_bucket(split->target_seg, seg, 
						already_seg, new, bucket_id, split->depth) == 0))
		return NULL;

	if (likely(&seg->bucket[0] != bucket))
